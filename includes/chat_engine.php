<?php
/**
 * Motor de chat de Daniel (copiloto con tool use), compartido entre:
 *  - api/chat.php (chat web interno)
 *  - includes/inbox.php (conversación por WhatsApp con los administradores del equipo)
 * Daniel consulta CRM, pipeline, conocimiento y campañas; redacta y guarda borradores.
 * NUNCA envía a un cliente: deja todo para aprobar.
 */
require_once __DIR__ . '/db.php';
require_once __DIR__ . '/agent.php';        // trae outreach, knowledge, claude, whapi, mailer
require_once __DIR__ . '/leads_intel.php';
require_once __DIR__ . '/campaigns.php';

if (!function_exists('chat_tools')) {
...
function chat_tools(): array {
    return [
        ...
        ['name' => 'buscar_conocimiento', 'description' => 'Busca pasajes en la base de conocimiento de SISTEL (casos, propuestas, guías, productos SENSEI/6E).',
...
/** System para conversación por WhatsApp con un administrador del equipo. */
function chat_system_admin(string $name): string {
    $n = trim($name) !== '' ? trim($name) : 'un integrante del equipo';
    return chat_system()
        . "\n\nESTE CHAT ES POR WHATSAPP con {$n}, ADMINISTRADOR del equipo de SISTEL (NO es un prospecto: es tu gente de confianza). "
        . "Atendelo como su asistente comercial: respondé sus consultas sobre la operación, el pipeline, los prospectos, las campañas y el conocimiento de SISTEL, y ayudalo a gestionar. "
        . "Formato WhatsApp: mensajes breves, claros y directos, sin firmas ni encabezados de correo. Tuteo cálido y de colega.";
}

/** Genera la respuesta del asistente para un admin (con herramientas). */
function chat_admin_reply(string $adminName, array $messages): array {
    return claude_call_tools(chat_system_admin($adminName), $messages, chat_tools(), 'chat_execute', 6, 1500);
}
